...
metadat=read.csv("../../maits.metadata.csv", header=TRUE)
...
barplot(sort(sample_sums(ps.itsx.no_nc))); abline(h=15000, col='red'); abline(h=12531, col='blue'); abline(h=15995, col='purple')
...
#                   202 OTUs were removed because they are no longer 
...
ps.itsx.no_nc.raref.filt=subset_taxa(ps.itsx.no_nc.raref, !is.na(Phylum)) # now 425 taxa 


## threshold filtering i.e. ASVs <1% abundance in at least one sample}
library(metagMisc)
ps.itsx.no_nc.raref.filt.gt1perc=phyloseq_filter_sample_wise_abund_trim(ps.itsx.no_nc.raref.filt, minabund = 125, relabund = FALSE, rm_zero_OTUs = TRUE)  # 125.31 reads is 1% of 12,531 (which is what smaples are raref to)
tax_table(ps.itsx.no_nc.raref.filt.gt1perc) # 34 ASVs now
...
if(length(w.cult) > 0) {
  ...
  tax_table(ps.itsx.no_nc.raref.filt.gt1percOther)[w.cult,] 
}
# ## 3. glom by genus
ps.itsx.no_nc.raref.filt.gt1percOther.gglom=tax_glom(ps.itsx.no_nc.raref.filt.gt1percOther, "Genus" , NArm=TRUE)
ps.itsx.no_nc.raref.filt.gt1percOther.gglom     # now 10 taxa
tax_table(ps.itsx.no_nc.raref.filt.gt1percOther.gglom)[,"Genus"]


# relative abundance
ps.itsx.no_nc.raref.filt.gt1percOther.gglom.prop <- transform_sample_counts(ps.itsx.no_nc.raref.filt.gt1percOther.gglom, function(otu) otu/sum(otu))
...
otucounts3 <- otucounts3 %>% dplyr::select(sampleID,starts_with("ASV"))  %>% pivot_longer(-sampleID, values_to = "count", names_to = "ASV")
...
########## Fig 1 A FINAL
## jitter plot
set.seed(14);taxonrelabund3.mafilt %>%
  group_by(sampleID, taxon) %>% 
  ...
  mutate(med_rel_abund = median(rel_abund)) %>%
